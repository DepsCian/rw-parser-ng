...
export class DffParser extends RwFile {
  ...
  parse(): RwDff {
    ...
    if (
      ...
      geometryList.geometries.length === 0
    ) {
  ...
  public readGeometry(versionNumber: number): RwGeometry {
    ...
    let binMesh: RwBinMesh | undefined = undefined;
    let skin: RwSkin | undefined = undefined;

    let relativePosition = 0;
    while (relativePosition < sectionSize) {
      const header = this.readSectionHeader();
      relativePosition += header.sectionSize + 12;
      const sectionStart = this.getPosition();
      ...
      this.setPosition(sectionStart + header.sectionSize);
    }
