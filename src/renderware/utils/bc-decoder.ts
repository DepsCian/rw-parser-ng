import { decode565, readUInt16LE, readUInt32LE } from "./color-decoder";
...
export function bc1(data: Uint8Array, width: number, height: number): Uint8Array {
  ...
  for (let y = 0; y < height; y += 4) {
    for (let x = 0; x < width; x += 4) {
      ...
      if (color0 > color1) {
        colorPalette[8] = (2 * c0r + c1r) / 3; colorPalette[9] = (2 * c0g + c1g) / 3; colorPalette[10] = (2 * c0b + c1b) / 3;
        colorPalette[12] = (c0r + 2 * c1r) / 3; colorPalette[13] = (c0g + 2 * c1g) / 3; colorPalette[14] = (c0b + 2 * c1b) / 3;
      } else {
        colorPalette[8] = (c0r + c1r) >> 1; colorPalette[9] = (c0g + c1g) >> 1; colorPalette[10] = (c0b + c1b) >> 1;
...
export function bc2(data: Uint8Array, width: number, height: number, premultiplied: boolean): Uint8Array {
  const rgba = new Uint8Array(4 * width * height);
  const colorPalette = new Uint8Array(16);
...
export function bc3(data: Uint8Array, width: number, height: number, premultiplied: boolean): Uint8Array {
  const rgba = new Uint8Array(4 * width * height);
  ...
  for (let y = 0; y < height; y += 4) {
    for (let x = 0; x < width; x += 4) {
      ...
      if (alpha0 > alpha1) {
        alphaPalette[0] = alpha0; alphaPalette[1] = alpha1;
        ...
        alphaPalette[6] = 0; alphaPalette[7] = 255;
      }
